...
export default function Shop({ onNavigate, coins, ownedCars, ownedDrivers, onBuyCar, onBuyDriver }: ShopProps) {
  ...
  const availableCars = useMemo(() => {
    return (Object.keys(CARS) as CarId[])
    ...
    <div className="h-full bg-slate-900 text-white p-6 overflow-y-auto">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-12">
          {tab === 'cars' ? (
            availableCars.map((id) => {
              const car = CARS[id];
              const isOwned = (ownedCars || []).includes(id);
              const canAfford = coins >= car.cost;

              return (
                <div key={`shop-car-${id}`} className={`bg-slate-800 rounded-2xl p-6 border-2 ${isOwned ? 'border-green-500/50' : 'border-slate-700'} relative overflow-hidden`}>
            ...
            availableDrivers.map((id) => {
              ...
              return (
                <div key={`shop-driver-${id}`} className={`bg-slate-800 rounded-2xl p-6 border-2 ${isOwned ? 'border-green-500/50' : 'border-slate-700'} relative overflow-hidden`}>
                  <div className="flex justify-between items-start mb-4">
                    <div>
